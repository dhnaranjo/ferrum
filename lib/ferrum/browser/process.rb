    ...
    class Process
      ...
      def initialize(options)
        ...
        if options.url
          ...
          url.query = options.query_params
          response = JSON.parse(::Net::HTTP.get(url))

          ws_url = URI.parse(response["webSocketDebuggerUrl"])
          ws_url.scheme = "wss"
          ws_url.query = options.query_params
          self.ws_url = ws_url.to_s
